...
func (p *ProcInfo) GetStatus() (err error) {
	...
	n, err := fmt.Sscanf(string(statBuf),
		...
		&p.Stat.Minflt, &p.Stat.Cminflt, &p.Stat.Majflt, &p.Stat.Cmajflt,
		&p.Stat.Utime, &p.Stat.Stime, &p.Stat.Cutime, &p.Stat.Cstime,
		&p.Stat.Priority, &p.Stat.Nice, &p.Stat.NumThreads, &p.Stat.Itrealvalue, &p.Stat.Starttime,
		&p.Stat.Vsize, &p.Stat.Rss, &p.Stat.Rsslim,
		&p.Stat.Startcode, &p.Stat.Endcode, &p.Stat.Startstack,
		&p.Stat.Kstkesp, &p.Stat.Kstkeip, &p.Stat.Signal, &p.Stat.Blocked, &p.Stat.Sigignore, &p.Stat.Sigcatch, &p.Stat.Wchan, &p.Stat.Nswap, &p.Stat.Cnswap,
		&p.Stat.ExitSignal, &p.Stat.Processor, &p.Stat.RtPriority, &p.Stat.Policy, &p.Stat.DelayacctBlkioTicks, &p.Stat.GuestTime, &p.Stat.CguestTime,
		&p.Stat.StartData, &p.Stat.EndData, &p.Stat.StartBrk, &p.Stat.ArgStart, &p.Stat.ArgEnd, &p.Stat.EnvStart, &p.Stat.EnvEnd,
		&p.Stat.ExitCode,
	)
	if err != nil {
		fmt.Println(err)
